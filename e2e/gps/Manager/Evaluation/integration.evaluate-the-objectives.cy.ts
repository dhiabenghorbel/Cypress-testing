...
const GPConfig: IGPConfig = {
  id: "ASW03OP1",
  menuPath: { topic: "Evaluation", name: "Evaluate the objectives" }
};
  ...
  describe("should load the app and display the home page content", () => {
              ...
              cy.getByDataForm("ASF03OCP")
                ...
                .then(() => {
                  if (results["FLGOAL"] === "1") {
                    ...
                    cy.getByDataForm("ASF03NAS").should("be.visible");
                    ...
                    cy.getRecordsFromTable("ZY3N")
                      ...
                      .then(() => {
                        if (lengthZY3N > 0) {
                          ...
                          cy.get('[data-test-id="DTTARG"] input')
                            .should("be.visible")
                            .should("have.value", results["DTTARG"]);
                          cy.getBySel("RTWGHT")
                            .should("be.visible")
                            .should("have.value", results["RTWGHT"]);
                          cy.getBySel("TXOBSH")
                            .should("be.visible")
                            .should("have.value", results["TXOBSH"]);
                          cy.getBySel("FLPROB")
                            .should("be.visible")
                            .should("have.value", results["FLPROB_EXT"]);
                          cy.getBySel("TXOBLG")
                            .should("be.visible")
                            .should("have.value", results["TXOBLG"]);
                          if (results["FLGOMS"] === "1") {
                            cy.getBySel("TXMESR")
                              .should("be.visible")
                              .should("have.value", results["TXMESR"]);
                          }
                          if (results["FLMDRW"] === "1") {
                            cy.getBySel("STCOMP")
                              .should("be.visible")
                              .should("have.value", results["STCOMP"]);
                          }
                          cy.getBySel("RTMANA")
                            .should("be.visible")
                            .clear()
                            ...
                            cy.getBySel("TXRWCM")
                              ...
                              .then(comment => {
                                if (comment.val() != "") {
                                  ...
                                  cy.wrap(comment).type(fetchGpData.dataToTypeManager["TXRWCM"]);
                                }
                              });
                          }
                  ...
                  if (results["FLSCRT"] === "1") {
                    ...
                    if (results["TYFLGL"] === "LNEAR") {
                      ...
                      cy.getBySel("RTPRGL")
                        .should("be.visible")
                        .type(fetchGpData.dataToTypeManager["RTPRGL"]);
                    }
                  }
                  ...
                  if (results["FLSCTK"] === "1") {
                    ...
                    if (results["TYFLTK"] === "LNEAR") {
                      cy.getBySel("RTLNTK")
                        .click()
                        .should("be.visible")
                        .select(fetchGpData.dataToTypeManager["RTLNTK"]);
                    } else if (results["TYFLTK"] === "GRADE") {
                      cy.getBySel("RTGRTS")
                        .click()
                        .should("be.visible")
                        .select(fetchGpData.dataToTypeManager["RTGRTS"]);
                  ...
                  cy.getByDataForm("ASF03OCP")
                    ...
                    .then(() => {
                      if (results["FLGOAL"] === "1") {
                        ...
                        cy.getByDataForm("ASF03NG1").should("be.visible");
    ...
    it("should switch to employee role and validate the task", () => {
      ...
      cy.wait(`@${GPConfig.id}`).then(({ response: { body } }: any) => {
        ...
        cy.get(".subtitle-container")
          .contains("Campaign Description")
          .should("be.visible");

        cy.getByDataForm("ASF03OGS").should("be.visible");
        cy.getBySel("STROBJ")
          .should("be.visible")
          .should("have.value", results["STROBJ"])
          .then(() => {
            if (results["FLGOAL"] === "1") {
              cy.get(".subtitle-container")
                .contains("Objectives")
            ...
            cy.get('[data-test-id="BLOB01"]').then(blob => {
              if (blob.find('[data-test-id="file-blob-name"]').length === 0) {
                return;
              } else {
                cy.getBlobnameFromDataSection("ZY3O")
          ...
          .then(() => {
            ...
            if (results["FLSCTK"] === "1") {
              ...
              if (results["TYFLTK"] === "LNEAR") {
                cy.getBySel("RTLNTK")
                  .should("be.visible")
                  .should("have.value", results["RTLNTK"]);
              } else if (results["TYFLTK"] === "GRADE") {
                cy.getBySel("RTGRTS")
                  .should("be.visible")
                  .should("have.value", results["RTGRTS"]);
              } else if (results["TYFLTK"] === "PRCNT") {
                cy.getBySel("RTPRTS")
                  .should("be.visible")
            ...
            if (results["FLOWPO"] === "1") {
              ...
              cy.getBySel("STMENT")
                .should("be.visible")
                .should("have.value", results["STMENT_EXT"]);
            }
            if (results["FLPFCR"] === "1") {
              cy.get(".subtitle-container")
                .contains("Other criteria")
    ...
    it("should switch to manager role and validate the task", () => {
      ...
      cy.wait(`@${GPConfig.id}`).then(({ response: { body } }: any) => {
        ...
        cy.getBySelFromForm("ASTWT1A5", "STATUX")
          .should("be.visible")
